...
def build_fp_tree(dataset, min_support):
    ...
    for transaction in dataset:
        ...
        for item in transaction:
            current_node = insert_node(item, current_node, header_table, 1)
...
def update_header_table(item, target_node, header_table):
    if header_table[item][1] is None:
        ...
        current_node = header_table[item][1][-1]
...
def extract_frequent_patterns(header_table, prefix=[]):
    ...
    for item, node in header_table.items():
        ...
        frequent_patterns.append(current_pattern)
        ...
        while node.node_link is not None:
            ...
            while node.parent is not None:
                ...
                node = node.parent
            if prefix_path:
                conditional_database[tuple(prefix_path)] = node.count
        ...
        if conditional_fp_tree is not None:
            ...
            frequent_patterns.extend(conditional_patterns)
